...
\begin{figure}[ht!]
    ...
    \caption{Diagrama showing the reconstruction algortihm workflow for electrons and photons, extacted from \Refn{\cite{ATLASEGammaPerformance20152017}}}
...
The variables with good discriminating power used are described in \Refn{\cite{ATLASMuonPerformance2016}}. Four identification selections are defined: Loose, Medium, Tight, and High-\pt. The first three categories are inclusive, and Medium being the default selection in \ac{ATLAS}. Finally, the muon candidates to be used by the analyses are asked to satisfy the isolation requirements, both at traack and calorimetric levels, analogously to what was detailed for photons in the previous section. For the first case, a variable similar to that used for photons is used, but with a variable-radius cone \(\DeltaR = \min(10~\gev/\pt, 0.3)\) around the muon \pt, excluding the muon track. For calorimetric isolation the same variable \etconefo is used, with the difference of using a radius of \(R=0.2\), instead of \(0.4\) as before. Based on these variables, 7 isolation selection criteria (7 \acp{WP}), optimized for different analyses, are defined.
...
% In case of large-\(R\) jets, these pile-up contributions affect the angular distributions needed for the reconstruction of the invariant mass, and are removed with a technique called \textit{grooming}, described in \Refn{\cite{JetTrimming}}. For small-\(R\) jets, in the first place, a correction of the origin of its vertex is performed and then the pile-up contribution is suppressed using methods that take into account the pile-up energy density~\cite{ATLASJetPileupRun1}, along with the variables associated to the tracks and \ac{PV}~\cite{ATLASJESUncertaintiesRun2}.
